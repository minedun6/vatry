...
$.ajax({
    ...
    method: 'POST',
    dataType: 'json',
    data: {typetransfert: 'parisAirport'},
...
function getTarif() {
    ...
    $.ajax({
        url: Routing.generate('calculate_tarif_paris_airport'),
        method: 'POST',
...
function initDetailsBox(datePicker, volDestSelect, flightSelect, flightHiddenSelector, volTimeInput, zipCodeSelected, direction, rdvMsgContainer, index) {
    ...
    $(datePicker).datepicker({
        ...
        // faire en sorte que ca devienne 1 pour un role particulier
    ...
    $(flightSelect).on('change', function () {
        ...
        if ($(flightSelect).val() == '') {
            ...
            if (direction == 'from') {
                ...
                rdvDate = substractMinutesToTime(volDetails.pickUpTime, 240);
            }
...
function setLieuRdvLieuDepot() {
    ...
    var gareDepotLocation1 = gareDepotLocation2 = "RER / Metro Bibliothèque François Mitterrand (Sortie Avenue de France) 117 Av de France 75013 Paris";
    ...
    if ($("#transfer_direction").val() == 'to_vatry') {
        console.log(gareDepotLocation1);
        //Lieu depot
