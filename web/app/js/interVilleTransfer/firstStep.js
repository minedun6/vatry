...
function initDetailsBox(datePicker, volDestSelect, flightSelect, flightHiddenSelector, volTimeInput, direction, numBlock) {
    ...
    $(datePicker).on('change', function () {
        ...
        var url = null;
        if (direction == 'to') {
            url = Routing.generate("get_destinations",{ 'type' : 'interville' });
        } else {
            url = Routing.generate("get_provenances",{ 'type' : 'interville' })
        }
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (receivedData) {
                //console.log($(locationSelector).val());
    ...
    $(flightSelect).on('change', function () {
        ...
        if ($(flightSelect).val() == '') {
            ...
            if ($(volTimeInput).hasClass('flag')) {
                HRDV = substractMinutesToTime(volDetails.pickUpTime, 120 + duration);
            } else {
                HRDV = addMinutesToTime(volDetails.pickUpTime, 35);
            }
...
$(function () {
    ...
    $('#inter_ville_transfer_first_step_time').attr('readonly', true).clockpicker({autoclose: true});
    $('#depot-heure').attr('readonly', true).clockpicker({autoclose: true});
